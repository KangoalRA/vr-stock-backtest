...
# --- 페이지 설정 ---
st.set_page_config(page_title="라오어 전략 끝판왕 백테스트", layout="wide")

st.title("📊 무매법(V1, V2.2, V3.0) vs VR vs DCA 통합 분석")
st.info("사용자가 제공한 V3.0 전/후반전 및 쿼터매도 로직과 VR(G=10, Band=15%)이 완벽히 적용되었습니다.")

# --- 사이드바 설정 ---
st.sidebar.header("📝 기본 설정")
ticker = st.sidebar.selectbox("티커 (Ticker)", ["TQQQ", "SOXL", "UPRO", "BITU", "TSLA", "NVDA"])
...
split_count = st.sidebar.number_input("원금 분할 횟수", value=40)
vr_g = st.sidebar.number_input("VR G값 (기울기 %)", value=10.0)
vr_band = st.sidebar.number_input("VR 밴드값 (폭 %)", value=15.0)
...
# 1. 무한매수법 V1.0 (기초형: 0.5 평단LOC + 0.5 시장가 매수)
def run_v1(df, initial_cap, splits, monthly_amt, dep_day):
    ...
    for i in range(len(df)):
        ...
        if shares > 0 and (p - avg_p)/avg_p >= 0.10: # +10% 전량 매도
            cash += (shares * p) + wait_cash; shares, avg_p, wait_cash = 0, 0, 0
            budget = cash / splits
        # 매수: 0.5는 평단LOC, 0.5는 무조건 매수 (간소화하여 1회분 매수로 처리)
        if cash >= budget:
            cnt = budget / p
...
def run_v22(df, initial_cap, splits, monthly_amt, dep_day):
    ...
    for i in range(len(df)):
        ...
        if shares > 0 and (p - avg_p)/avg_p >= 0.10: # 익절 로직
...
# 3. 무한매수법 V3.0 (완성형: 전/후반전 & 쿼터매도)
def run_v3(df, initial_cap, splits, monthly_amt, dep_day):
    cash, wait_cash, shares, avg_p, last_m, acc_buy = initial_cap, 0, 0, 0, -1, 0
    budget = cash / splits
    equity = []
    for i in range(len(df)):
        ...
        # [매도 로직] +15% 졸업 및 후반전 쿼터 매도
        if shares > 0:
            profit_rate = (p - avg_p) / avg_p
            if profit_rate >= 0.15: # +15% 전량 매도
                cash += (shares * p); shares, avg_p, acc_buy = 0, 0, 0
            elif t_val >= splits / 2: # 후반전 리스크 관리: 쿼터 매도 (25%를 평단가 본절 탈출)
                if p >= avg_p:
                    sell_q = shares * 0.25
...
# 4. VR (Value Rebalancing: G=10, Band=15%)
...
if run_btn:
    ...
    if not df.empty:
        ...
        res['무매법 V1.0'] = run_v1(df, initial_capital, split_count, monthly_amount, deposit_day)
        res['무매법 V2.2'] = run_v22(df, initial_capital, split_count, monthly_amount, deposit_day)
        res['무매법 V3.0'] = run_v3(df, initial_capital, split_count, monthly_amount, deposit_day)
        res['VR 전략'] = run_vr(df, initial_capital, vr_g, vr_band, monthly_amount, deposit_day)
        ...
        tab1, tab2 = st.tabs(["💰 자산 수익금 추이", "📉 전략별 MDD (개별 라인)"])
        with tab1:
            fig1 = px.line(res, x=res.index, y=res.columns, title=f"{ticker} 전략별 평가액 비교")
